class Deque {
  ...
  table() {
    ...
    const data = [0, ...this.data.data[this.step], 0];
    if (this.step) {
      if (this.data.data[this.step - 1].length < this.data.data[this.step].length) {
        const datal = [...this.data.data[this.step]]; datal.shift();
        const datar = [...this.data.data[this.step]]; datar.pop();
        if (JSON.stringify(datal) == JSON.stringify(this.data.data[this.step - 1])) {
          data[0] = svg.arrowr;
        }
        if (JSON.stringify(datar) == JSON.stringify(this.data.data[this.step - 1])) {
          data[data.length - 1] = svg.arrowl;
        }
      }
      if (this.data.data[this.step - 1].length > this.data.data[this.step].length) {
        const datal = [...this.data.data[this.step - 1]]; datal.shift();
        const datar = [...this.data.data[this.step - 1]]; datar.pop();
        if (JSON.stringify(datal) == JSON.stringify(this.data.data[this.step])) {
          data[0] = svg.arrowl;
        }
        if (JSON.stringify(datar) == JSON.stringify(this.data.data[this.step])) {
          data[data.length - 1] = svg.arrowr;
        }
      }
      if (data[0]) {
        const D = data[0];
      ...
      if (data[data.length - 1]) {
        ...
        const X = (data.length - 1 + 0.5) * width / data.length;
        const Y = height / 2;
        const S = width / data.length;
        fill(theme.sbg); stroke(theme.bg);
        square(X, Y, S);
